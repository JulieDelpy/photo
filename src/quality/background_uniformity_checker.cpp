...
public:
    ...
    CheckResult check(const cv::Mat& image, const FaceInfo& face,
        ...
        int sw = std::max(w / 8, 15), sh = std::max(h / 8, 15);
        ...
        result.max_threshold = kCornerRangeWarn;

        // 背景均匀度仅作 WARNING：
        // 自然灯光渐变在 pass 图片中也普遍存在，无法用角采样可靠区分
        if (corner_range > kCornerRangeWarn) {
            result.passed = false;
            result.severity = Severity::WARNING;
            result.message = "背景存在渐变/阴影: 四角差值="
                           + std::to_string(static_cast<int>(corner_range))
                           + " (均值 " + std::to_string(static_cast<int>(min_mean))
                           + "~" + std::to_string(static_cast<int>(max_mean)) + ")";
        } else {
            result.passed = true;
            result.severity = Severity::PASS;
            result.message = "背景均匀: 四角差="
                           + std::to_string(static_cast<int>(corner_range));
        }
        return result;
    }
